# FeatherFence 个人稳定分支架构说明

本文档描述个人仓库 `personal/v11-stable` 的当前实现，而不是尚未落地的目标蓝图。该分支从
`c9ab9f1` 分出，首个个人预发行版为 `v0.1.4-personal.1`：`cargo test --all-targets` 共 71 项通过，
`RUSTFLAGS="-D warnings"` 的 release 构建成功。个人 `main` 继续保留上游基线。
...
- D；
- 超出 v11 安全边界的桌面直接落点扩张（多项目、复制、跨卷、重名或非空白目标强制定位）；
- 新的桌面整理功能扩张；
...
│  └─ drop_position.rs      # 拖出目标校验、桌面定位、绘制锁和布局保存
├─ fence/
...
| `DesktopVisualLockGuard` | 桌面绘制锁代次 | 释放匹配代次的绘制锁并重绘桌面 |
...
`transfer/drag_source.rs` 仍由 Explorer 执行真实文件 MOVE/COPY；个人版只在单项目、同卷、无重名、
桌面空白单元格、未启用自动排列且使用普通拖动或 `Shift` 的情况下启用快速路径：

1. 栅栏在进入 `DoDragDrop` 前先隐藏源条目，避免栅栏内外短暂重复；
2. 数据对象提供真实 `DROPFILES.pt` 和 `CFSTR_SHELLIDLISTOFFSET`；
3. `IDropSourceNotify` 确认当前目标属于桌面 ListView；
4. `desktop/drop_position.rs` 短暂锁住桌面绘制，等待 Explorer 建立目标项目；
5. 通过 `IFolderView::SelectAndPositionItems` 定位，并用 `IShellView::SaveViewState` 保存布局；
6. 正常路径、失败路径和两秒看门狗都会释放绘制锁。

`Ctrl` 复制、跨卷、重名、非空白目标、自动排列、COM/坐标获取失败或目标不确定时不接管坐标，回退
Explorer 原生行为。后续从桌面移走另一个项目之前，会先保存当前桌面视图，避免用户手动调整的其他
图标恢复到旧落点。
...
| 桌面落点任务 | 短生命周期 STA 工作线程 | 只持有任务参数和线程内取得的 Shell COM 接口；由代次淘汰旧任务 |
...
10. D、新桌面整理功能和超出 v11 回退边界的桌面定位扩张，在重新评估前不进入个人稳定分支。
...
| P1 | `48761ed` | Zen 热键可配置、可禁用并支持原子替换 |
| P2 | `0f5f834` | 受限桌面快速落点、重复图标抑制和布局回弹修复 |

PR #1–#30 已合并到个人 `main`；P1、P2 只存在于 `personal/v11-stable`，是个人预发行版已经落地的
功能，不是仍待实施的目标。
...
- `cargo fmt --check` 通过；
- `cargo test --all-targets`：71/71 通过；
- `RUSTFLAGS="-D warnings" cargo build --release` 通过；
- Clippy 的旧 `main` 告警数量不再直接当作 v11 基线；若开始清理，先在当前分支重新计数；
- 启动桌面层级曾完成多轮冷启动人工验证；资源和文件生命周期由真实 Win32/NTFS 测试覆盖。
- `Ctrl+Shift+Z`、单项目桌面鼠标落点、重复图标抑制和后续布局不回弹已在当前机器人工通过。

剩余事项按优先级重新评估，不自动视为下一项功能。具体状态、保留分支性质和新工作准入条件见
[`remaining-work.md`](remaining-work.md)：

- 清理 Clippy 遗留告警时必须按机械、小批次 PR 处理；
- `main.rs` 和 `Fence` 若继续拆分，应以减少依赖和字段可见性为目标，不机械搬文件；
- 个人 `main` 的旧稳定标签为 `personal-v0.1.4-stable-20260827`；当前个人版以
  `v0.1.4-personal.1` 和 `personal/v11-stable` 为发行、开发基线；
- 视觉层级、Explorer 重启和多显示器行为仍需在涉及对应代码时做最小人工验证；
- D 和超出 v11 安全边界的桌面定位扩张保持暂停，除非重新提出明确需求、边界和验证方案。

## 12. PR 验证标准

每个后续 PR 至少满足：

- 从最新 `personal/v11-stable` 创建；若目标是上游 PR，则另外从最新个人 `main` 拆出最小提交；
- 一个明确目的，不依赖未合并分支；
- `git diff --check` 和 `cargo fmt --check` 通过；
- 全部单元测试通过，纯重构不减少测试；
- 若涉及 Clippy，先记录当前分支基线并保证告警不增加；
- release 构建通过；

...
- **拖入 / 拖出** —— 通过 OLE `IDropTarget` 把桌面或资源管理器的文件拖进栅栏，也可从栅栏拖出；个人版会在条件明确的同卷单项目 MOVE 中按鼠标位置提交桌面图标，并避免短暂重复与后续布局回弹，其余情况回退 Explorer 原生行为。
...
- **拖出到桌面** —— 普通拖动或按住 `Shift` 将单个项目移动到桌面空白处时，个人版会尽力按鼠标释放位置放置；`Ctrl` 复制、跨卷、重名、非空白目标、自动排列或目标不确定时交给 Explorer 原生处理。
...
src/
  ...
  desktop/drop_position.rs 桌面拖出目标校验、图标定位、绘制锁和布局保存
  config.rs            JSON 迁移、原子保存、有效备份和损坏恢复
  hotkey.rs            全局热键解析、注册、替换与自动注销
